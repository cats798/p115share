...
class TGService:
    ...
    async def set_commands(self):
        ...
        commands = [
            ...
            BotCommand(command="clearhistory", description="清除所有历史记录"),
            BotCommand(command="history", description="查看最近50条历史记录"),
        ]
    ...
    async def _cleanup_bot(self, bot_instance=None):
        ...
        if target_bot:
            try:
                ...
                # Cancel all pending verify tasks
                ...
                try:
                    ...
                    logger.debug(f"{prefix} Webhook 删除超时，跳过")
                except Exception as ex:
                    logger.debug(f"{prefix} Webhook 删除失败: {ex}")
                ...
                if hasattr(target_bot, 'session') and target_bot.session:
                    try:
                        ...
                        logger.debug(f"{prefix} ✅ HTTP 会话已关闭")
                    except Exception as ex:
                        logger.debug(f"{prefix} HTTP 会话关闭出错: {ex}")
            except Exception as e:
                logger.error(f"{prefix} ❌ 清理过程中发生错误: {e}")
            finally:
                if not bot_instance:
                    self.bot = None
                    self.dp = None
                    self.is_connected = False
                    logger.debug(f"{prefix} 状态变量已重置")
    ...
    def _register_handlers(self):
        ...
        self.dp.message(Command("clearhistory"))(self.handle_clearhistory)
        self.dp.message(Command("history"))(self.handle_history)
    ...
    async def handle_clearhistory(self, message: types.Message):
        allowed = self._get_allowed_chats()
        if allowed and str(message.chat.id) not in allowed:
            logger.warning(f"Unauthorized chat access attempt for /clearhistory: {message.chat.id}")
            return
        success = await p115_service.delete_all_history_links()
        if success:
            await message.answer("✅ 所有历史记录已清除")
        else:
            await message.answer("❌ 清除历史记录失败")

    async def handle_history(self, message: types.Message):
        allowed = self._get_allowed_chats()
        if allowed and str(message.chat.id) not in allowed:
            logger.warning(f"Unauthorized chat access attempt for /history: {message.chat.id}")
            return
        records = await p115_service.get_all_history_links(limit=50)
        if not records:
        ...
        for r in records[:10]:
            ...
            if share_link.startswith('[') and share_link.endswith(']'):
                try:
                    links = json.loads(share_link)
                    share_link = '\n'.join(links)
                except:
                    pass
            lines.append(f"• 原链接: {r['original_url'][:50]}...\n  新分享: {share_link}")
        if len(records) > 10:
    ...
    async def handle_message(self, message: types.Message):
        ...
        async def process_single_link(share_url, index, segment_info=None):
            try:
                ...
                metadata = {
                    ...
                    "title": segment_info["title"] if segment_info else None
                }
                ...
                if save_res:
                    if save_res.get("status") == "success":
                        ...
                        if reason == "snapshotting" and total_links == 1:
        ...
        # Prepare segments with title extraction
        ...
        for idx, pos in enumerate(link_positions):
            ...
            if idx < len(link_positions) - 1:
                ...
                elif len(between_text.strip()) > 10:
        ...
        for i, url in enumerate(share_urls, 1):
            ...
            if res is True:
                ...
                if target_segment:
                    ...
                    await self.broadcast_to_channels(
                        {url: share_link},
                        {"full_text": full_text, "entities": ser_entities, "photo_id": photo.file_id if photo else None}
                    )
            elif res == "pending":
                pending_count += 1
            else:
        ...
        if total_links == 1:
            if success_count == 1:
                ...
                await status_msg.edit_text(error_text if error_text.startswith("❌ 处理完成") else error_text)
        else:
            result_text = f"✅ 批量处理完成！\n\n成功: {success_count}\n"
            if pending_count:
                result_text += f"⏳ 审核/快照中: {pending_count}\n"
            if failed_count:
        ...
        if settings.TG_USER_ID and str(message.chat.id) != str(settings.TG_USER_ID):
            try:
                ...
                if failed_count:
                    ...
                    if failed_details:
                        admin_msg += "\n失败详情：\n"
                        for idx, (failed_url, error_msg) in enumerate(failed_details[:3], 1):
    ...
    async def poll_pending_link(self, message: types.Message, pending_info: dict):
        ...
        interval = 1800 if reason == "snapshotting" else 3600 if reason == "restricted" else 300
        max_attempts = 36
    ...
    async def _post_to_single_channel_batch(self, channel_config: dict, share_links_map: dict, metadata: dict):
        ...
        try:
            ...
            if photo_id:
                ...
                await self.bot.send_photo(channel_id, photo=photo_id, caption=new_text, caption_entities=new_entities)
            else:
                await self.bot.send_message(channel_id, text=new_text, entities=new_entities, disable_web_page_preview=False)
    ...
    def _update_access_codes(self, text: str, entities: list, share_links_map: dict) -> tuple[str, list]:
        ...
        for old_url in sorted_originals:
            ...
            if isinstance(share_link_val, list):
                if not share_link_val: continue
